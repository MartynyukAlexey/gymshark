...
func (s *Service) Confirm(req *ConfirmReq) error {
	...
	if err != nil {
		...
		s.Logger.Error("/service/auth/confirm: failed to get user by email", "err", err)
		return err
	}
	...
	if err != nil {
		s.Logger.Error("/service/auth/confirm: failed to get confirmation codes", "err", err)
		return err
	}
	...
	for _, code := range codes {
		if err := bcrypt.CompareHashAndPassword(code.Hash, []byte(req.Code)); err != nil {
			if err != bcrypt.ErrMismatchedHashAndPassword {
				s.Logger.Error("/service/auth/confirm: failed to verify confirmation code", "err", err)
				return err
			}
		} else {
			// code matches
			...
			if err := s.Storage.User.UpdateStatus(user.ID, models.UserStateActive); err != nil {
				s.Logger.Error("/service/auth/confirm: failed to activate user account", "err", err)
				return err
			}
