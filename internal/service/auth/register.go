...
func (s *Service) Register(req *RegisterReq) (uuid.UUID, error) {
	...
	if err != nil {
		s.Logger.Error("/service/auth/register: failed to hash password", "err", err)
		return uuid.Nil, err
	}
	...
	if err := s.Storage.User.DeleteByEmailIfInactive(req.Email); err != nil {
		s.Logger.Error("/service/auth/register: failed to delete old non-active user", "err", err)
		return uuid.Nil, err
	}
	...
	if err := s.Storage.User.Insert(m); err != nil {
		...
		s.Logger.Error("/service/auth/register: failed to save user", "err", err)
		return uuid.Nil, err
	}

	code, err := generateCode(8)
	if err != nil {
		s.Logger.Error("/service/auth/register: failed to generate activation code", "err", err)
		return uuid.Nil, err
	}

	codeHash, err := bcrypt.GenerateFromPassword([]byte(code), bcrypt.DefaultCost)
	if err != nil {
		s.Logger.Error("/service/auth/register: failed to hash activation code", "err", err)
		return uuid.Nil, err
	}
	...
	if err := s.Storage.Code.Insert(t); err != nil {
		s.Logger.Error("/service/auth/register: failed to save activation code", "err", err)
		return uuid.Nil, err
	}

	go func() {
		if err := s.Mailer.SendActivationEmail(m.Email, code); err != nil {
			s.Logger.Error("/service/auth/register: failed to send activation email", "err", err)
		}
	}()
